...
TEAM = {'student':'乌萨斯学生自治团','rainbow':'彩虹小队','followers':'使徒','laios':'莱欧斯小队','mujica':'Ave Mujica','lee':'鲤氏侦探事务所','chiave':'贾维团伙','action4':'行动组A4','reserve6':'预备行动组A6'}
...
for cid, v in chartab.items():
    ...
    if v.get('isNotObtainable'):
        continue
    r = v.get('rarity')
    if r not in ('TIER_4', 'TIER_5', 'TIER_6'):
        continue
...
# 实装日期：banner 与创建时间 reconcile（取较早者，差异>30 天以创建时间为准并记录）
n_date = 0
for o in ops:
